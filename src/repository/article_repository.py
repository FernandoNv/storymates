...
from src.repository.db import db
...
class ArticleRepository():
  ...
  def get_all_by_author(self, username):
    return list(filter(lambda item: item["author"] == username, db))
  ...
  def __findById(self, id_article):
    index_article = next((i for i, article in enumerate(db) if article['id'] == id_article), None)
